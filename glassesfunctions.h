...
void cycleBrightness() {
  ...
  if (currentBrightness > 204) {
    currentBrightness = 51;
  } else {
      currentBrightness += 51;
  }
  
  byte newBrightness = getCIE(currentBrightness);
...
void XYPWM(int8_t x, int8_t y, byte value) {

  if (x >= 0 && x < kMatrixWidth && y >= 0 && y < kMatrixHeight) {
    if (GlassesPWM[x][y] < value) GlassesPWM[x][y] = value;
  }
